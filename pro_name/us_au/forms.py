...
class Register(forms.Form):
	...
	emailid = forms.CharField(label='email',max_length=20,required=True)
	bio = forms.CharField(label='user bio',max_length=20,required=True,widget=forms.Textarea)
	date_of_birth = forms.CharField(label='DOB',required=True,widget=forms.DateInput(attrs={'type':'date'}))
	
	class Meta:
		model = User
		fields = ('Username','first_name','last_name','password1','password2','date_of_birth','emailid','bio')
